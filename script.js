// Initial list of default members
...
// Initialize variables from localStorage or use default members
...
function saveToLocalStorage() {
    ...
    // Add timestamp to force storage event
    localStorage.setItem('lastUpdate', new Date().getTime());
}

// Update the addBatchmateForm event listener
addBatchmateForm.addEventListener('submit', (e) => {
    ...
    // Add new batchmate with an ID
    const newId = (members.length + 1).toString().padStart(2, '0');
    members.push({ id: newId, name, phone });
    saveToLocalStorage(); // Save after adding new member
...
// Update the removeBatchmate function
function removeBatchmate(phone) {
    if (confirm('Are you sure you want to remove this batchmate?')) {
        ...
        if (index !== -1) {
            ...
            // Reorder IDs after removal
            members.forEach((member, idx) => {
                member.id = (idx + 1).toString().padStart(2, '0');
            });
            
            saveToLocalStorage(); // Save after removing member
            updateLists();
...
// Update the attendance form submission handler
attendanceForm.addEventListener('submit', (e) => {
    ...
    saveToLocalStorage(); // Save after marking attendance
...
resetButton.addEventListener('click', () => {
    if (confirm('Are you sure you want to reset attendance?')) {
        presentMembers = [];
        saveToLocalStorage(); // Save after resetting attendance
...
window.addEventListener('storage', (e) => {
    if (e.key === 'presentMembers' || e.key === 'members') {
...
setInterval(() => {
    const storedPresentMembers = JSON.parse(localStorage.getItem('presentMembers')) || [];
    const storedMembers = JSON.parse(localStorage.getItem('members')) || defaultMembers;

    // Check if there are any changes
    if (JSON.stringify(storedPresentMembers) !== JSON.stringify(presentMembers) ||
        JSON.stringify(storedMembers) !== JSON.stringify(members)) {
        
        // Update local data
        presentMembers = storedPresentMembers;
